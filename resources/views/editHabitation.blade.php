...
@section('content')
    <div class="detailsPost__container">
        @if(\Illuminate\Support\Facades\Auth::check())
            <form style="float: right" method="post" action="{{route('deleteHabitation', $habitation->id)}}">
                @csrf
                @method('delete')
            ...
            <form method="post" action="{{route('updateHabitation', $habitation->id)}}" enctype="multipart/form-data">
                @csrf
                @method('put')
                <div class="form-group">
                    <label>Nom</label>
                    <input type="text" class="form-control" name="nom" value="{{$habitation->nom}}" required>
                </div>
                <div class="form-group">
